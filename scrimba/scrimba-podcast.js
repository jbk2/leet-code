...
import data from './scrimba-podcast-data.js'

function sortByDuration(data, flightLength){
  
  const direction = flightLength <= 60 ? 1 : -1;

  data.sort((a, b) => direction * (a['duration'] - b['duration']));

  data.forEach((pod, i) => {
    console.log(`${i + 1}. ${pod['title']}, ${pod['duration']} minutes`)
  })
}

console.log(sortByDuration(data, 60));
// // 